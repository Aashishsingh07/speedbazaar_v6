...
type Product = { id: string; name: string; category: string; price: number; stock: number; seller: string };
...
export default function AdminProductsPage() {
  ...
  const submit = async () => {
    const res = await fetch("/api/products", {
      ...
      body: JSON.stringify({ ...form, price: Number(form.price), stock: Number(form.stock), isFeatured: true }),
    });
    const data = await res.json();
      ...
      <main className="auth-shell">
        ...
        <div className="grid grid-2">
          <div className="card section-card">
            <h3 style={{ marginTop: 0 }}>Add Product Manually</h3>
            <div className="grid">
          ...
          <div className="card section-card">
            ...
            {products.length === 0 ? <p className="muted small">No products yet.</p> : (
              <table className="table">
                <thead><tr><th>Name</th><th>Seller</th><th>Category</th><th>Price</th><th>Stock</th></tr></thead>
                <tbody>
                  {products.map((p) => <tr key={p.id}><td>{p.name}</td><td>{p.seller}</td><td>{p.category}</td><td>₹{p.price}</td><td>{p.stock}</td></tr>)}
                </tbody>
              </table>
            )}
          </div>
